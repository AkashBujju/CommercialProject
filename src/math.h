...
float f_normalize(float value, float from_range_start, float from_range_end, float to_range_start, float to_range_end); Vector4 mul_vec_and_mat4(Vector4 *vec, Matrix4 *mat);
...
uint32_t obb(Matrix4 *models, uint32_t count, struct BoundingBox *bounding_boxes, Vector *ray);
